...
class Graph:
    ...
    def bfs(self, initial_node, visited):
        ...
        visited.add(initial_node)  # Define an initial node, marking it as visited
    ...
    def get_unvisited_nodes(self, visited):
        return set(self.graph.nodes).difference(visited) # return unvisited nodes
    ...
    def run(self):
        visited = set()  # Visited list initialization
        ...
        while unvisited:  # While there are unvisited nodes
            print("\n\t> Start", end="")
            node = random.choice(list(unvisited))  # Randomly pick an unvisited neighbour
            print(node)
